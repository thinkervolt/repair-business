...
@section('page')
{{ __('repair-business.invoice_settings') }}
@endsection
@section('page-content')
    <div class="container-fluid">
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">{{ __('repair-business.invoice_settings') }}</h1>
        </div>
        <div class="row">
            <div class="col">
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h5 class="h5 mb-0 text-gray-700">{{ __('repair-business.status') }}</h5>
                </div>
                <form action="{{ route('create-setting-invoice') }}" method="POST">
                    ...
                    <div class="form-group row m-0">
                        <label for="color" class="col-sm-4 col-form-label">{{ __('repair-business.input_color') }}</label>
                        <div class="col-sm-8">
                    ...
                    <div class="form-group row m-0">
                        <label for="group" class="col-sm-4 col-form-label">{{ __('repair-business.input_group') }}</label>
                        <div class="col-sm-8">
                                ...
                                @foreach ($statuses as $setting)
                                    <tr>
                                        ...
                                        <form method="POST" action="{{ route('delete-setting-invoice', $setting) }}">
                                            @csrf
                                            @method('DELETE')
                                            <td class="text-right"><button type="submit" class="btn btn-danger  btn-sm"><i
                                                        class="fas fa-trash"></i> {{ __('repair-business.button_delete') }}</button></td>
                                        </form>
                                    </tr>
                                @endforeach
