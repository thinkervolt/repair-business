...
<div class="print-area-container">
    ...
    <div class="data-block">
        ...
        <p class="data-line">{{preg_replace("/^(\d{3})(\d{3})(\d{4})$/", "$1-$2-$3", $invoice->customer_phone)}}</p>
  ...
  @endif
    ...
    <tr>
        <td></td>
        <td colspan="2">
            <p class="data-line right">TAX ( {{number_format((float)$invoice->tax_porcentage, 2, '.', ',')}}%)</p>
        </td>
        <td> 
    ...
    <p class="small-data-line right">{{ date_format($payment->created_at,"M d, Y h:iA")}}</p>
   
    </td>
    <td>
  
    
    <p class="data-line right">${{number_format((float)$payment->amount, 2, '.', ',')}}</p>
    </td>
